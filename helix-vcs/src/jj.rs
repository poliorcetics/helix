...
pub(super) fn get_diff_base(repo: &Path, file: &Path) -> Result<Vec<u8>> {
    ...
    /// Helper function to run `jj diff` with the same default arguments
    fn run_jj_diff(
        repo: &Path,
        file: &Path,
        ...
        let output = jj_command(repo)
            .args([
                ...
                option.as_ref(),
                value.as_ref(),
                // If the filepath starts with `-` or `--` it could be interpreted as an option or
                // flag, which is not what the user will want and could at worst allow malicious
                // repos to run destructive commands.
                "--".as_ref(),
        ...
        anyhow::ensure!(
            ...
            "`jj diff` executed but failed for {file:?}"
        );

        Ok(output.stdout)
    }

    let stdout = run_jj_diff(
        repo,
        file,
        // Check if file is newly added, existing or unmodified
        "--template",
        "status_char",
    )?;

    match stdout.trim_ascii().first().copied() {
        // File existed in previous change and has no diff
        None => std::fs::read(file).context("could not read jj diff base from existing file"),
        // File is new from current change
        Some(b'A' | b'C') => Ok(Vec::new()),
        // File existed in previous change and has diff, in which case we need to get the diff base
        Some(_) => {
            ...
            std::fs::read(tmppath).context("could not read jj diff base from target")
        }
    }
}

pub(crate) fn get_current_head_name(repo: &Path) -> Result<Arc<ArcSwap<Box<str>>>> {
    let out = jj_command(repo)
        .args([
            "log",
            "--no-graph",
            ...
            "immutable_heads()::@ & bookmarks()",
            "--template",
            // See <https://docs.jj-vcs.dev/latest/templates/>
            //
            ...
            //
            // We use a short ID with 8 characters because in practice the change ID is extremely
            // unlikely to conflict since we only consider mutable commits (like most jj commands
            // will do by default) and this leaves space for bookmarks to appear in the status bar
            // even on narrower screens.
            r#"change_id.short(8) ++ " " ++ bookmarks ++ "\n""#,
        ])
        .output()?;
...
pub(crate) fn for_each_changed_file(
    ...
    // ```
    // C:text.txt //
    // D:added // copied.txt //  // copied.txt // file //
    // D:renamed // to-rename.txt // file // renamed.txt // file //
    // D:removed // to-delete.txt // file // to-delete.txt //  //
    // D:modified // to-modify.txt // file // to-modify.txt // file //
    // ```
    //
    ...
    let template = r#"concat(
        conflicted_files
            .map(|file| concat("C:", file.path().display(), " //\n"))
            .join(""),
        diff
            .files()
            ...
            .map(|file| concat(
                ...
                " // ",
                file.source().file_type(),
                " // ",
                file.target().path().display(),
                " // ",
                file.target().file_type(),
                " //\n",
            ))
            .join(""),
    )"#;
    ...
    if !out.status.success() {
        log::warn!("`jj show` executed but failed:\n{out:#?}");
        anyhow::bail!("`jj show` executed but failed");
    }

    for entry in split_double_slash(&out.stdout, true) {
        let change = match entry {
            [b'C', b':', rest @ ..] => FileChange::Conflict {
                path: make_pathbuf(rest),
            },
            [b'D', b':', rest @ ..] => match entry_to_change(rest) {
                Some(change) => change,
                None => continue,
            },
            _ => continue,
...
pub(crate) fn open_repo(repo_path: &Path) -> Result<()> {
    assert!(
        ...
        "no .jj where one was expected: {repo_path:?}",
    );

    // Checking that the .jj we found is actually a JJ repo
    let status = jj_command(repo_path)
        .args(["workspace", "root"])
...
/// Prepare a JJ command with the common boilerplate
fn jj_command(repo: &Path) -> Command {
    // Prevents argument injection when the repo is named `--my-repo`.
    // The public interface in `./lib.rs` will canonicalize before trying to look for a JJ repo
    // so this is here to avoid issues in case of refactors
    assert!(
        repo.is_absolute(),
        "only absolute paths can be used as jj repo paths"
    );
...
fn extract_head_name(output: &str) -> Result<String> {
    ...
    let head_text = if !exact_bookmarks.is_empty() {
        ...
        let ancestor_bookmarks = std::iter::from_fn(next)
            .map(|e| e.1)
            .filter(|s| !s.is_empty())
            .collect::<Vec<_>>();
        if ancestor_bookmarks.is_empty() {
            // Found no bookmarks amongst ancestors.
            rev.to_string()
        } else {
            // Angle brackets: bookmarks are on ancestors.
            format!("{rev} [{}]", ancestor_bookmarks.join(" ").trim())
        }
    };
